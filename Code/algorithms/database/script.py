...
def add_mint_data_from_file(file_name, db):
    ...
    df1 = df.head(5)
    taxonA = df1['Taxon A'].unique()
    taxonB = df1['Taxon B'].unique()
    ...
    for taxon in taxon_list['_id']:
        print(taxon)
        proteinA = df1[df1['Taxon A'] == taxon].iloc[:, :3]
        ...
        proteinB = df1[df1['Taxon B'] == taxon].iloc[:, 3:6]
...
#     PPIDb = Database()
    ...
    for gene in gene_list:
        print(get_affinity(gene, cutoff=1000))
