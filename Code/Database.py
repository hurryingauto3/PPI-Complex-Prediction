...
class Data:
    def __init__(self) -> None:
        self.interactions = {
            "Source": "",
            "databaseID": "",  # the id of the interaction in the database source for the interaction
            "GeneA": "",  # the official symbol A for protein A
            "GeneB": "",  # the official symbol A for protein B
            "mintScore": "",  # MINT score for the interaction
            "dataType": "",  # primary or secondary data (produced by our algo)
            "ExperimentID": "",  # _id in the experiment table for the interaction

            # Need to add descriptions of the following fields:
            "interactionLength": "",  # length of interaction,
            "interactionType": "",  # type of interaction,
            "interactionWeight": "",  # weight of interaction,,
            "interactionSize": "",  # size of interaction,
            "interactionSASA": "",  # surface area of interaction,
            "interactionShape": "",  # shape of interaction,
            "interactionDensity": "",  # density of interaction,
            "interactionComplementarity": "",  # complementarity of interaction,
            "interactionRIP": "",  # residue surface propensity,"
            "interactionHydrophobicity": "",  # hydrophobicity of interaction,"
        ...
        self.proteins = {
            ...
            # Need to add descriptions for each of the following:
            "avgProtienLength": "",
            "maxProtienLength": "",
            "minProtienLength": "",
            "avgProtienWeight": "",
    ...
    def set_inters(self, source, db_id, GeneA, GeneB, score, type, pred_score, exp_id):
        self.interactions["Source"] = source
        self.interactions["Database ID"] = db_id
        self.interactions["GeneA"] = GeneA
        self.interactions["Gene B"] = GeneB
        self.interactions["MINT Score"] = score
        self.interactions["Type of data"] = type
        self.interactions["Predicted Score"] = pred_score
        self.interactions["ExperimentID"] = exp_id

    def set_proteins(self, gene, uniprotKB, taxon, desc):
        self.proteins["_id"] = gene
        self.proteins["Gene"] = gene
        self.proteins["UniprotKB AC"] = uniprotKB
        self.proteins["TaxonID"] = taxon
        self.proteins["Description"] = desc

    def set_taxons(self, taxon, species):
        self.taxon["_id"] = taxon
        self.taxon["TaxonID"] = taxon
        self.taxon["Species Name"] = species

    def set_exps(self, sys, sys_type, author, pubmed):
        self.experiment["Experimental System"] = sys
        self.experiment["Experiment System Type"] = sys_type
        self.experiment["Author"] = author
        self.experiment["Publication Source (PubMed ID)"] = pubmed
...
class Database:
    ...
    def get_interactions_by_protien(self, protien):
        return self.interactions.find({"GeneA": protien}, {"Gene B": protien})
    ...
    def get_interactions_by_species(self, species_name):
        taxon_id = self.taxonomy.find_one(
            {"Species Name": species_name})["TaxonID"]
        proteins = self.proteins.find({"TaxonID": taxon_id})
        interactions = []
...
def add_mint_data(file_name, db):
    ...
    for line in file.readlines():
        data = Data()
        # Protein A;GeneA;Taxon A;Protein B;Gene B;Taxon B;Score;PMID
        temp = line.split(";")
...
def add_mentha_data(file_name, db, species):
    ...
    for line in file.readlines():
        data = Data()
        # Protein A;GeneA;Protein B;Gene B;Score;mis
        temp = line.split(";")
